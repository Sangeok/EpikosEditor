...
export default function Preview() {
  ...
  const handleNext = () => {
    setCurrentImageIndex((prev) => (prev === imageData.length - 1 ? 0 : prev + 1));
  };

  const handlePrevious = () => {
    setCurrentImageIndex((prev) => (prev === 0 ? imageData.length - 1 : prev - 1));
  };

  const hasImage = imageData?.length > 0;
  const canTurnOverImage = imageData.length >= 2;
