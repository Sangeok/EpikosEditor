...
const SCRIPT_PROMPT_LIFE_SCIENCE_EN = `
...
const SCRIPT_PROMPT_LIFE_SCIENCE_KO = `
You are an expert viral YouTube Shorts scriptwriter who reveals the hidden science behind everyday life and keeps viewers watching to the very end.
Write two different scripts for a 45 second video.
...
export async function POST(req: Request) {
  ...
  if (topic === "Philosophy") {
    ...
    if (language === "English") {
      PROMPT = SCRIPT_PROMPT_LIFE_SCIENCE_EN.replace("{life science}", topicDetail);
    } else {
      PROMPT = SCRIPT_PROMPT_LIFE_SCIENCE_KO.replace("{life science}", topicDetail).replace("{language}", language);
    }
  }
